...
export default function ProductCard({
  ...
  const starArray = Array(5)
    .fill(null)
    .map((_, index) => chooseStar(index));
  const [starProgress, setStarProgress] = useState(starArray);

  return (
    <div className="ProductCard">
      {oos ? <span className="oos-marker">Out of stock</span> : null}
      <img
        src={img}
        alt={name}
        className={`product-img ${oos ? "oos-image" : ""}`}
      />
      ...
      <div className="rating-cont">
        <div className="star-container">{starProgress}</div>
      ...
      <div className="price-buy-container">
        <div className="price-container">
          {discount ? (
            <p className="price-value old-price-value">{oldPrice} Lei</p>
          ) : null}
          <p className="price-value">{price} Lei</p>
        </div>
        <button className={`buy-item-btn ${oos ? "blocked" : ""}`}>
          <svg
            className="icon shopping-cart-icon"
            data-slot="icon"
            fill="none"
            stroke-width="1.5"
            viewBox="0 0 24 24"
